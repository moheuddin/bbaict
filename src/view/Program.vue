    ...
    <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <v-card>
                ...
                <v-row>
                    ...
                    <v-col cols="12" sm="6">
                        <v-menu ref="menu2" v-model="menu2" :close-on-content-click="false" :nudge-right="40" :return-value.sync="time" transition="scale-transition" offset-y max-width="290px" min-width="290px">
                            ...
                            <v-time-picker v-if="menu2" v-model="editedItem.time" full-width @click:minute="$refs.menu2.save(time)"></v-time-picker>
                        </v-menu>
                    </v-col>
                    <v-col cols="12" sm="12">
                        <v-text-field name="program" v-validate="'required'" v-model="editedItem.program" label="Program"></v-text-field>
                        <span v-show="errors.has('program')" class="text-danger">{{ errors.first('program') }}</span>
                    </v-col>
                    <v-col cols="12" sm="12">
                        <v-text-field name="place" v-validate="'required'" v-model="editedItem.place" label="Place"></v-text-field>
            ...
            <v-card-actions>
                ...
                <v-btn color="success" text :disabled="loading" @click="saveItem(editedItem)">Save
    ...
    export default {
        ...
        methods: {
            ...
            loadItems(filter = '') {
                ...
                axios.get(config.API_URL + 'program.php', {
                    params: {
                        data
                    }
                }).then((response) => {
            ...
            deleteItem(item) {
                ...
                if (confirm('Are you sure you want to delete this?')) {
                    ...
                    this.loading = true;
                    axios.delete(config.API_URL + 'program.php', {
            ...
            saveItem(item) {
                ...
                axios.post(config.API_URL + 'program.php', {
                    data: item
                }).then((response) => {
